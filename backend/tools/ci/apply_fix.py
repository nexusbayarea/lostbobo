...
from backend.tools.ci.fix_engine import generate_fix_script, propose_fix, validate_fix
...
def run_ci_in_sandbox(sandbox_path: str) -> bool:
    ...
    if result.returncode == 0:
        print("[SANDBOX] CI passed in sandbox")
        return True
...
def apply_and_validate(root_cause: dict, repo_path: str = None) -> dict:
    ...
    if not validate_fix(fix):
        print("[SANDBOX] Fix requires manual review")
        return {
            "status": "NO_FIX",
            "fix": fix,
