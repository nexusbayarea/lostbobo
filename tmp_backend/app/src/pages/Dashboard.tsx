...
export function Dashboard() {
    ...
    pollInterval.current = window.setInterval(async () => {
      try {
        ...
        if (status.status === 'completed') {
          ...
          if (pollInterval.current) clearInterval(pollInterval.current);
          setIsRunning(false);
          toast.error('Simulation analysis failed.');
        }
      } catch (error) {
  ...
  const handleStartRun = async (overrides?: { method: string; numRuns: number }) => {
    try {
      ...
      setProgress({ current: 0, total: activeNumRuns });
      ...
      const response = await api.startRobustnessRun(config);
  ...
  const handleCancelRun = async () => {
    ...
    try {
      await api.cancelRobustnessRun(currentRunId);
      if (pollInterval.current) clearInterval(pollInterval.current);
      setIsRunning(false);
      setCurrentRunId(null);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex">
      ...
      <motion.aside
        ...
        animate={{
          ...
          // Only shift off-screen on mobile (screen < 1024px)
      ...
      <main
        className={cn(
          ...
          'lg:ml-0', // Default
          isSidebarOpen ? 'lg:pl-[280px]' : 'lg:pl-[80px]' // Use padding instead of margin to keep it centered if needed, or margin
        )}
        style={{ marginLeft: 0 }} // Override the fixed margin
      >
        {/* Header */}
