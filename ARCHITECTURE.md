...
> Status: **LIVE (v2.7.0)** — Docker consolidation, single port
...
### The "Mission Control" Stack (v2.7)

For local development, use the unified `docker-compose.yml` in `docker/compose/`:

| Service | Dockerfile | Description |
| :--- | :--- | :--- |
| API (Unified) | `docker/images/Dockerfile.unified` | FastAPI + Worker + Autoscaler on port 8080 |
| Frontend | — (React/Vite) | Unified React/Vite cockpit in `frontend/` |
| Supabase | — (Cloud) | Database + Auth (source of truth) |
| RunPod | GraphQL podReset | GPU pod management |

**v2.7 Changes:**
- Removed Redis (Supabase is source of truth)
- Single port 8080 (removed 8000 backup)
- Consolidated Dockerfiles to `/docker/images/`
- Docker Compose updated: `docker/compose/docker-compose.yml`
...
- **`frontend/`** (Git submodule): Contains the React/Vite application. This directory is a gitlink pointing into `lostbobo.git`. Both the submodule and the parent monorepo share the same remote (`lostbobo.git` → Vercel deployment source). Vercel auto-deploys on push to `lostbobo.git` main.
...
├── legacy_archive/          # Deprecated v1.6.0-ALPHA & frontend artifacts
...
## Appendix: Mission Control - The Operational Cockpit
...
- **Fleet Dashboard:** Admin view at `/api/v1/admin/fleet` showing running/stopped pods, queue depth, daily cost, and scaling events. Features a **"Wake GPU"** manual override to resume pods from disk hibernation in ~90s.
