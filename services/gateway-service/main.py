"""
SimHPC Gateway Service — API ingress, auth, routing, streaming orchestration.
Integrates the kernel orchestrator, connects to RAG service, module runtime, and simulation node.
"""
...
SIM_HOST = os.getenv("SIMULATION_HOST", "simulation-runtime")
SIM_PORT = os.getenv("SIMULATION_PORT", "50053")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Gateway started, services connected")
    yield
    await clients.close()


app = FastAPI(title="SimHPC Gamma Gateway", version="1.0.0", lifespan=lifespan)
